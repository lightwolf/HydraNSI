...
//
#ifndef HDNSI_SAMPLER_H
#define HDNSI_SAMPLER_H
...
// Define template specializations of HdNSITypeHelper methods for
// all our supported types...
#define TYPE_HELPER(T,type)\
template<> inline HdTupleType \
HdNSITypeHelper::GetTupleType<T>() { return HdTupleType{type, 1}; }
...
public:
    ...
    HdNSIBufferSampler(HdVtBufferSource const& buffer)
        : _buffer(buffer) {}
    ...
    template<typename T> bool Sample(int index, T* value) const {
        return Sample(index, static_cast<void*>(value),
            HdNSITypeHelper::GetTupleType<T>());
    }
...
public:
    /// Default constructor.
    HdNSIPrimvarSampler() = default;
    /// Default destructor.
    virtual ~HdNSIPrimvarSampler() = default;
