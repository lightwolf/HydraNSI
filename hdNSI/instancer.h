...
/// \class HdNSIInstancer
///
/// HdNSI implements instancing by adding prototype geometry to the BVH
/// multiple times within HdNSIMesh::Sync(). The only instance-varying
/// attribute that HdNSI supports is transform, so the natural
...
///
class HdNSIInstancer : public HdInstancer {
public:
    /// Constructor.
    ...
    HdNSIInstancer(HdSceneDelegate* delegate, SdfPath const& id,
...
private:
    ...
    // by HdNSIMesh::Sync(), which is dispatched in parallel, so it needs
